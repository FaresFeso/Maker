...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('send-panel')
        .setDescription('ارسال بانل التكت'),
    async execute(interaction) {
        ...
        if (!guilddata) return interaction.editReply({ content: `**لم يتم تحديد الاعدادات**` });

        let embedmessage = guilddata.embedmessage;
        ...
        if (!panelsRoom || !panel1Category || !panel1Role || !panel1Welcome || !panel1Name || !panel2Category || !panel2Role || !panel2Welcome || !panel2Name) {
            return interaction.editReply({ content: `**لم يتم تحديد الاعدادات بشكل صحيح**` });
        }

        let panelroom = interaction.guild.channels.cache.get(panelsRoom);

        if (!panelroom) {
            return interaction.editReply({ content: `**لا استطيع العثور على روم البانل**` });
        }
    ...
    await panelroom.send({embeds: [embed], components:[row]})
